...
public class NIFParser {
  ...
  public String createTextWithMentions(final String text, final List<Span> mentions) {
    ...
    for (int i = 0; i < mentions.size(); i++) {
      final Span span = mentions.get(i);

      final int begin = span.getStartPosition();
      final int end = begin + span.getLength();

      if (begin < lastPos) {
        // we have two overlapping mentions --> take the larger one
        final Span prev = mentions.get(i - 1);
      ...
      final String before = text.substring(lastPos, begin);
      final String label = text.substring(begin, end);
      lastPos = end;
      textBuilder.append(before).append("<entity>").append(label).append("</entity>");
    }

    final String lastSnippet = text.substring(lastPos, text.length());
